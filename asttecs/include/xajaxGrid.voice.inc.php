...
class ScrollTable{
	...
	function ScrollTable($cols, $start = 0, $limit, $filter = null, $numRows = 0, $content = null, $order = null,$specfooter=null){
		...
		$this->start = $start;
		$this->top = '<table class="adminlist" border="3">';
		$this->rowStyle = "row1";
		$this->filter = $filter;
...
*/
		...
		$this->header .= '
				<th style="text-align: center" class="'.$class.'" width="5%" nowrap>
					'.$local_grid->Translate("Play").'
				</th>';

		if($delete) // uncomet for delete header
			$this->header .= '
				<th style="text-align: center" class="'.$class.'" width="5%" nowrap>
...
function addRow($table,$arr,$edit=true,$delete=true,$detail=true,$divName="grid",$fields=null,$filename)
{
		global $local_grid;
	   ...
	   foreach ($arr as $key => $value) 
	   {
		...
		if($edit)
		$row .= '<td id="'.$nameCell.'" style="cursor: pointer;" '.$this->colAttrib[$ind-1].'><a href="?" onClick="xajax_playmonitor('.$arr[0].');return false;" class="info"><img src="skin/default/images/play.png" border="0"><span>'.basename($value).'</span></a></td>'."\n";
		
		if($delete)
			$row .= '
					<td align="center" width="5%" nowrap>
						<a href="?" onClick="if (confirm(\''.$local_grid->Translate("Are you sure you want to delete this record").'\'))  searchFormSubmit(0,5,\''.$arr[0].'\',\'delete\');return false;"><img src="skin/default/images/index.jpg" border="0"></a>
					</td>';
		if($detail)
		{
			if($detail == 'customer')
			...
			{    
				$filename1 = explode("_", $filename);
	$datetime = explode("-",$filename1['0']);	
	$date1= $datetime['0']; 
	...
	$monthName = date("F", mktime(0, 0, 0, $month, 10)); //echo $monthName;
				...
				$dir1="voicefiles/".$year."/".$monthName."/".$day."/".$filename;
			     $row .= '<td id="'.$nameCell.'" style="cursor: pointer;" '.$this->colAttrib[$ind-1].'>   <a href="'.$dir1.'"> <img src="skin/default/images/Download.png" border="0"></a></td>'."\n";
				//$row .= '<td id="'.$nameCell.'" style="cursor: pointer;" '.$this->colAttrib[$ind-1].'><a href="?" onClick="xajax_playmonitor1('.$arr[0].');return false;" class="info"><img src="skin/default/images/Download.png" border="0"><span>'.basename($value).'</span></a></td>'."\n";
	...
	function addRoww($table,$arr,$edit=true,$delete=true,$detail=true,$divName="grid",$fields=null){
		global $local_grid;
		$nameRow = $divName."Row".$arr[0];
	    $row = '<tr id="'.$nameRow.'" class="'.$this->rowStyle.'" >'."\n";
		$ind = 0;
	   ...
	   foreach ($arr as $key => $value) {
		...
		if($edit)
			$row .= '
					<td align="center" width="5%" nowrap>
						<a href="?" onClick="xajax_edit('.$arr[0].',\''.$table.'\');return false;"><img src="skin/default/images/play.png" border="0"></a>
					</td>';
		if($delete)
			$row .= '
					<td align="center" width="5%" nowrap>
						<a href="?" onClick="if (confirm(\''.$local_grid->Translate("Are you sure you want to delete the record").'\'))  searchFormSubmit(0,5,\''.$arr[0].'\',\'delete\');return false;"><img src="skin/default/images/index.jpg" border="0"></a>
					</td>';
		if($detail){
			if($detail == 'customer'){
				$row .= '
	...
	function addRowSearchMore($table,$fieldsFromSearch,$fieldsFromSearchShowAs,$filter,$content,$start,$limit, $withNewButton = 1,$typeFromSearch = null,$typeFromSearchShowAs = null,$stype = null){
		...
		$this->search = '
			...
			<tr>
				...
				if($withNewButton){
					$this->search .= '<input type="hidden" id="addButton" value="'.$local_grid->Translate("add_record").'" onClick="xajax_add();return false;">';
				}
	/*	$this->search .= '
				</td>
				<td> '.$local_grid->Translate("table").': ';
			...
			for($j=0;$j<count($filter);$j++){
				if(trim($content[$j]) != '' && trim($filter[$j]) != ''){
					...
					<select name="searchField[]" id="searchField[]">
									<option value="'.null.'">'.$local_grid->Translate("Select").'</option>';
		...
		$this->search .= '</select>&nbsp;&nbsp;';
				...
				//$this->search .= "<img src=images/cal.gif onclick=javascript:NewCssCal('searchContent".$j."','yyyyMMdd'); style=cursor:pointer>";
		...
		if($this->deleteFlag != 1 && $this->exportFlag != 1 && $this->multiEditFlag != 1){
			...
			if($this->exportFlag == 1){
				$option .= '<option value="export">'.$local_grid->Translate("Download").'</option>';
				$optionOnchange .= 'if(this.value==\'export\'){document.getElementById(\'submitButton\').value=\''.$local_grid->Translate("Download").'\';}';
			}
...
class Table {
	...
	*/
	...
	function Topp($tableTitle = "tableTitle",$formId = "formDiv"){
		$table = '
