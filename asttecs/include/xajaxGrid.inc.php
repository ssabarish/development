...
class ScrollTable{
	...
	*/
	...
	function setHeader($class,$headers,$attribs,$events,$edit=true,$delete=true,$detail=true,$select){
	...
	*/
	...
	function addRoww($table,$arr,$edit=true,$delete=true,$detail=true,$divName="grid",$fields=null,$select=true){
		global $local_grid;
...
function addRow($table,$arr,$edit=true,$delete=true,$detail=true,$divName="grid",$fields=null,$select=true){
		global $local_grid;
		$nameRow = $divName."Row".$arr[0];
	    $row = '<tr id="'.$nameRow.'" class="'.$this->rowStyle.'" >'."\n";
		$ind = 0;
...
// expot the values to my rate by setting the margin and selling and tax and pulse rate 
function setvalues($rid,$gid)
{ global $local_grid;
$this->search='
<form name="setval" method="post" action="imortmyrate.php">
SET ALL &nbsp&nbsp&nbsp<input type="checkbox" name="all" value="ON">   Margin :&nbsp&nbsp<input type="text" size="5"  name="margin" value="">
Constant Value:&nbsp&nbsp <input type="text" name="con" size="5"  value="">
Tax :&nbsp&nbsp<input type="text" name="tax" size="5"  value="" >
Billing Block:&nbsp&nbsp<input type="text" name="pulse" size="5"  value="">
<input type="submit" value="Export To My Rate ">
</form> ';
	...
	function addRowSearchMore($table,$fieldsFromSearch,$fieldsFromSearchShowAs,$filter,$content,$start,$limit, $withNewButton = 1,$typeFromSearch = null,$typeFromSearchShowAs = null,$stype = null){
		...
		$this->search = '
			...
			<tr>
				...
				if($withNewButton){
					$this->search .= '<input type="button" id="addButton" value="'.$local_grid->Translate("Add").'" onClick="xajax_add();return false;">';
				}
		$this->search .= '
				</td>
				<td> '.$local_grid->Translate("").'';
		//$this->search .= $table ;
		$this->search .= '</td>
				<td align="right" >
